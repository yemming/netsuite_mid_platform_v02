...
import { Package, Scan, Plus, Minus, X, Save, Trash2, Eye, Calendar, Truck } from 'lucide-react';
import { posDB, POSItem, ReceivingItem, ReceivingRecord } from '@/lib/indexeddb-pos';
...
export default function POSReceivingPage() {
  ...
  const handleSelectProduct = (product: POSItem) => {
    const existingIndex = receivingItems.findIndex((ri) => ri.barcode === product.barcode);

    if (existingIndex >= 0) {
      const updated = [...receivingItems];
      updated[existingIndex].quantity += 1;
      setReceivingItems(updated);
      showToast(`已增加 ${product.name} 的數量`, 'success');
    } else {
      const receivingItem: ReceivingItem = {
        itemId: product.id!,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await posDB.createReceiving(receivingRecord);
      ...
      <div className="max-w-6xl mx-auto p-4">
        ...
        <div className="bg-white dark:bg-[#1a2332] border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-3 mb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-bold text-gray-900 dark:text-white">門店收貨</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 space-y-4">
              ...
              <CardContent>
                {receivingItems.length === 0 ? (
                  <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                    ...
                    <p>收貨清單是空的</p>
                    <p className="text-sm mt-1">請掃描條碼或選擇產品加入收貨清單</p>
                  </div>
                ) : (
                  <div className="space-y-3">
            ...
            <Card className="dark:bg-[#1a2332] dark:border-gray-700">
              <CardHeader className="pb-3">
                <CardTitle className="text-base dark:text-white">收貨資訊</CardTitle>
              </CardHeader>
